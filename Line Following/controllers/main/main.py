...
class Car():
    def __init__(self):
        ...
        self.COLOR_RANGE = {
            ...
            'BLACK': [(0, 0, 0), (60, 255, 255)],  # BLACK
        }
        self.wheels_names = ["motor1","motor2","motor3","motor4"]
        self.ds_names = ["ds1","ds2"]
        ...
        self.mDistanceSensor = []
        for i in range(2):
            self.mDistanceSensor.append(self.robot.getDevice(self.ds_names[i]))
            self.mDistanceSensor[i].enable(self.mTimeStep)
    ...
    # def LineFollowing(self):
        # while self.robot.step(self.mTimeStep*4) != -1:
            ...
            # if self.v1 < self.v2:
                # for i in range(4):
                    # self.speed1[i] += 20*self.speed_leftCircle[i] * (self.v2 - self.v1 ) / self.Mask.shape[1] + ((140-self.v1)/10)
            # else:
                # for i in range(4):
                    # self.speed1[i] += 20*self.speed_rightCircle[i] * (self.v1 - self.v2 ) / self.Mask.shape[1]+ ((self.v1-140)/7)
